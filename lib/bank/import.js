...
import { autoTag, ruleKey, SHARED_COST } from './categories.js';
...
function decide(tx, rules, jobs, books) {
  ...
  let split = translateSplit(guess.split, books);
  /* Damp's partners share the business's costs, so a categorised spend is
     everyone's unless the line says otherwise. A company's spend is the
     company's and needs no split at all. */
  if (books.model === 'damp' && !split.length && tx.amountPence < 0 && SHARED_COST.includes(guess.category)) {
    split = books.targets.map((t) => t.key).filter((k) => k !== 'tax');
  }
  return { key, category: guess.category, split, jobId: null, matchKind: null };
}
