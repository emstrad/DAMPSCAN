 ...
 * and this week, quotes out with no answer, money owed, payouts ready to
 * freeze, and frozen figures that have drifted. Each row carries enough to
...
export default async function handler(req, res) {
  ...
  try {
    const [e, v, q, o, p] = await Promise.all([enquiries(sites), visits(sites), quotes(sites), owed(moneySites), payouts(moneySites)]);
    json(res, 200, { ok: true, sites, money: moneySites.length > 0, enquiries: e, visits: v, quotes: q, owed: o, ready: p.ready, drifted: p.drifted });
  } catch (err) {
