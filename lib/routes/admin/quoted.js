...
import { businessFor, loadLines, engineInput, payoutRows, paymentState, present, QUOTED_STATUSES } from '../../quoted.js';
...
async function change(res, body, scope) {
  ...
  if (op === 'cost') {
    ...
    const rate = pence(body.dayRatePence);
    ...
    const rows = payoutRows(business.payout_model, job, result);
