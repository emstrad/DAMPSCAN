...
import { quotedEvent } from '../../notify-quoted.js';
...
/* The owner's phone hears about the changes that matter, and never a
   customer's details. Best effort: a push that fails never fails the write. */
async function tell(op, business, before, after) {
  const event = quotedEvent(op, business, before, after);
  if (event) await notify({ business: business.slug, ref: after.id, ...event });
}
...
async function save(res, body, scope) {
  ...
  if (id) {
    ...
    /* A save that does not mention the finder leaves the finder alone; one
       that names nobody clears it. Undefined and null are different answers. */
    row = await queryOne(
      `update jobs set customer_name = $2, customer_postcode = $3, note = $4, invoice_net_pence = $5, status = $6,
              job_date = coalesce($7::date, job_date), job_time = $8::time,
              finder_person_id = case when $10::boolean then $9::bigint else finder_person_id end, updated_at = now()
        where id = $1 returning *`,
      [id, fields.customer_name, fields.customer_postcode, fields.note, fields.invoice_net_pence, fields.status, fields.job_date, fields.job_time,
       fields.finder_person_id, body.finderPersonId !== undefined]
    );
  } else {
  ...
  await tell('save', business, before, after);
...
async function change(res, body, scope) {
  ...
  await tell(op, business, before, after);
  json(res, 200, { ok: true, job: after });
}
