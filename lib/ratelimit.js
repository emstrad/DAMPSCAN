...
export const LIMITS = {
  ...
  /* Generous enough that nobody filling in a form legitimately reaches it,
     tight enough that a script cannot run up somebody else's metered bill. */
  address: { bucket: 'address', limit: 40, windowSeconds: 600 },
  /* Ten files is the form's limit, so this allows a handful of retries and a
     second attempt at the booking. Anything past that is not a customer. */
  upload: { bucket: 'upload', limit: 20, windowSeconds: 600 },

  /* Ceilings across every address at once, for the two routes that spend
     money on our behalf: one is a metered lookup, the other writes up to 25MB
     into a store billed by the gigabyte. A per-IP limit alone is walked past
     with a pool of addresses, and neither route has a login to fail. Set at
     something like fifty times a busy day, so a person never meets them. */
  addressGlobal: { bucket: 'address_global', limit: 400, windowSeconds: 600 },
  uploadGlobal: { bucket: 'upload_global', limit: 200, windowSeconds: 600 }
};

/**
 * The per-IP limit and the global ceiling in one call, for the routes that
 * have both. Either being over is a refusal. Both counters are recorded, so an
 * abuser burning the global one is still counted against their own address.
 */
export async function rateLimitBoth({ perIp, global, ipHash }) {
  const [own, all] = await Promise.all([
    rateLimit({ ...perIp, ipHash }),
    rateLimit({ ...global, ipHash: 'all' })
  ]);
  return { ok: own.ok && all.ok, hits: own.hits, globalHits: all.hits, windowSeconds: perIp.windowSeconds };
}
