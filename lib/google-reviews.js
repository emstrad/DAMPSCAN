 ...
 * so below this the whole set is withheld, here rather than in the page, so
 * there is one rule rather than one per site.
 *
 * Four rather than five: DampScan's profile has four with usable text and no
 * more are coming, and four real reviews doubled across two tracks fills the
 * marquee perfectly well. Places returns at most five, so this never trims.
 */
