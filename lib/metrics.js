...
const build = (sql, range) => sql.replaceAll('$SINCE', sinceExpr(range));

/** Sessions that went on to book. Joined in wherever a booking rate is needed. */
const BOOKED = `(select distinct session_id from leads where stage = 'complete')`;
...
const COUNTERS = `
  ...
  from events where created_at >= $SINCE and ${VISITOR_ONLY}`;
...
const FORM_ERRORS = `
  select coalesce(detail->>'field', 'unknown') as field,
         ...
         count(distinct session_id)  as sessions
  from events
   where type = 'form_error' and created_at >= $SINCE
...
const CALL_LOG = `
  ...
  left join ${BOOKED} b on b.session_id = e.session_id
   where e.type = 'call_click' and e.created_at >= $SINCE
...
const breakdown = (keyExpr, having = '') => `
  ...
  from events e
  left join ${BOOKED} b on b.session_id = e.session_id
   where e.created_at >= $SINCE and e.${VISITOR_ONLY}
  group by 1
  ${having}
...
export async function summary(rangeInput) {
  const range = normaliseRange(rangeInput);
  const at = (sql) => build(sql, range);
