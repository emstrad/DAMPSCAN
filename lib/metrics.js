 ...
 */
/* A list now, because a request is scoped to the businesses its person may
   see. An empty list is `false`: nothing, rather than everything. */
const siteClause = (sites, prefix = '') => {
  const list = Array.isArray(sites) ? sites : (sites ? [sites] : null);
  if (list === null) return 'true';
  if (!list.length) return 'false';
  return `${prefix}site in (${list.map(quoteSite).join(', ')})`;
};
...
export async function summary(rangeInput, siteInput) {
  ...
  /* Either a single ?site= as before, or the list a scope allows. */
  const site = Array.isArray(siteInput) ? siteInput : normaliseSite(siteInput);
  ...
  return {
    ...
    counters: {
      ...
      partials,
      bookings,
      // A partial is now only written when someone abandons after step 1, so
      // it is a dropout count, not a stage every booking passes through.
      // Booking rate is therefore measured against sessions, not partials.
      sessionToPartial: rate(partials, sessions),
      sessionToBooking: rate(bookings, sessions)
    },
    funnel: {
      step1,
      step2,
