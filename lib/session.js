...
export function issueSession(res, user) {
  ...
  const token = signSession({
    ...
    name: user.name || null,
    role: user.role,
    /* True for a person who signed in with their own passcode. Absent for the
       shared code, which lib/access.js treats as the legacy damp admin. The
       scope itself is never stored here: it is read from grants per request,
       so a changed grant takes effect on the next click. */
    person: Boolean(user.person),
